...
namespace BimCore {
    ...
    struct Measurement { glm::vec3 p1; glm::vec3 p2; };
    struct Measurement2D { float p1[2]; float p2[2]; char text[64]; };

    struct SnapOverlay2D {
        bool     draw = false;
        SnapType type = SnapType::None;
        float    p[2]; float e0[2]; float e1[2];
    };

    struct SelectedObject {
        std::string guid; std::string type;
        uint32_t startIndex; uint32_t indexCount;
        std::map<std::string, PropertyInfo> properties;
    };

    struct SearchResult {
        uint32_t subMeshIndex; std::string matchType;
        std::string matchKey; std::string matchValue;
    };
    ...
    struct SelectionState {
        ...
        bool showCommandPanel   = false; // <--- NY VARIABEL FOR KOMMANDO PANELET
        ...
        // --- NYTT: Layout & Anchoring variabler for dynamiske menyer ---
        float uiMainPanelWidth       = 400.0f;
        float uiPropertiesPanelWidth = 350.0f; 
        float uiCommandPanelHeight   = 150.0f;
    };

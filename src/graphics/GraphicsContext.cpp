...
namespace BimCore {
    ...
    void GraphicsContext::CreateSurface(GLFWwindow* window) {
        ...
        #if defined(BIM_PLATFORM_WINDOWS)
        WGPUSurfaceSourceWindowsHWND hwndDesc = {}; hwndDesc.chain.sType = WGPUSType_SurfaceSourceWindowsHWND; hwndDesc.hinstance = GetModuleHandle(nullptr); hwndDesc.hwnd = glfwGetWin32Window(window); surfaceDesc.nextInChain = reinterpret_cast<const WGPUChainedStruct*>(&hwndDesc);
        #elif defined(BIM_PLATFORM_LINUX)
        const int platform = glfwGetPlatform(); WGPUSurfaceSourceWaylandSurface waylandDesc = {}; WGPUSurfaceSourceXlibWindow x11Desc = {};
        if (platform == GLFW_PLATFORM_WAYLAND) { waylandDesc.chain.sType = WGPUSType_SurfaceSourceWaylandSurface; waylandDesc.display = glfwGetWaylandDisplay(); waylandDesc.surface = glfwGetWaylandWindow(window); surfaceDesc.nextInChain = reinterpret_cast<const WGPUChainedStruct*>(&waylandDesc); }
        else if (platform == GLFW_PLATFORM_X11) { x11Desc.chain.sType = WGPUSType_SurfaceSourceXlibWindow; x11Desc.display = glfwGetX11Display(); x11Desc.window = glfwGetX11Window(window); surfaceDesc.nextInChain = reinterpret_cast<const WGPUChainedStruct*>(&x11Desc); }
        #endif
        m_surface = wgpuInstanceCreateSurface(m_instance, &surfaceDesc);
    }

    void GraphicsContext::RequestAdapterAndDevice() {
        WGPURequestAdapterOptions adapterOpts = {}; adapterOpts.compatibleSurface = m_surface;
        WGPURequestAdapterCallbackInfo adapterCb = {}; adapterCb.callback = [](WGPURequestAdapterStatus status, WGPUAdapter adapter, WGPUStringView, void* ud, void*) { if (status == WGPURequestAdapterStatus_Success) *static_cast<WGPUAdapter*>(ud) = adapter; }; adapterCb.userdata1 = &m_adapter;
        wgpuInstanceRequestAdapter(m_instance, &adapterOpts, adapterCb);
        WGPUDeviceDescriptor deviceDesc = {}; WGPURequestDeviceCallbackInfo deviceCb = {}; deviceCb.callback = [](WGPURequestDeviceStatus status, WGPUDevice device, WGPUStringView, void* ud, void*) { if (status == WGPURequestDeviceStatus_Success) *static_cast<WGPUDevice*>(ud) = device; }; deviceCb.userdata1 = &m_device;
        wgpuAdapterRequestDevice(m_adapter, &deviceDesc, deviceCb);
        m_queue = wgpuDeviceGetQueue(m_device);
    }

    void GraphicsContext::ConfigureSurface() {
        WGPUSurfaceCapabilities caps = {}; wgpuSurfaceGetCapabilities(m_surface, m_adapter, &caps);
        m_surfaceFormat = (caps.formatCount > 0) ? caps.formats[0] : WGPUTextureFormat_BGRA8Unorm;
        WGPUSurfaceConfiguration cfg = {}; cfg.device = m_device; cfg.format = m_surfaceFormat; cfg.alphaMode = (caps.alphaModeCount > 0) ? caps.alphaModes[0] : WGPUCompositeAlphaMode_Opaque; cfg.usage = WGPUTextureUsage_RenderAttachment; cfg.width = m_width; cfg.height = m_height; cfg.presentMode = WGPUPresentMode_Fifo;
        wgpuSurfaceConfigure(m_surface, &cfg); wgpuSurfaceCapabilitiesFreeMembers(caps);
    }

    void GraphicsContext::CreateRenderTargets() {
        ReleaseRenderTargets();

        WGPUTextureDescriptor cDesc = {}; cDesc.usage = WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding; cDesc.dimension = WGPUTextureDimension_2D; cDesc.size = { m_width, m_height, 1 }; cDesc.format = m_surfaceFormat; cDesc.mipLevelCount = 1; cDesc.sampleCount = 1;
        m_offscreenTexture = wgpuDeviceCreateTexture(m_device, &cDesc); m_offscreenView = wgpuTextureCreateView(m_offscreenTexture, nullptr);

        WGPUTextureDescriptor dDesc = {}; dDesc.usage = WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding; dDesc.dimension = WGPUTextureDimension_2D; dDesc.size = { m_width, m_height, 1 }; dDesc.format = WGPUTextureFormat_Depth24PlusStencil8; dDesc.mipLevelCount = 1; dDesc.sampleCount = 1;
        m_depthTexture = wgpuDeviceCreateTexture(m_device, &dDesc); m_depthView = wgpuTextureCreateView(m_depthTexture, nullptr);

        WGPUTextureViewDescriptor dReadDesc = {}; dReadDesc.format = WGPUTextureFormat_Undefined; dReadDesc.dimension = WGPUTextureViewDimension_2D; dReadDesc.baseMipLevel = 0; dReadDesc.mipLevelCount = 1; dReadDesc.baseArrayLayer = 0; dReadDesc.arrayLayerCount = 1; dReadDesc.aspect = WGPUTextureAspect_DepthOnly;
        m_depthReadView = wgpuTextureCreateView(m_depthTexture, &dReadDesc);

        if (!m_shadowTexture) {
            WGPUTextureDescriptor shadowDesc = {}; shadowDesc.usage = WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding; shadowDesc.dimension = WGPUTextureDimension_2D; shadowDesc.size = { 4096, 4096, 1 }; shadowDesc.format = WGPUTextureFormat_Depth32Float; shadowDesc.mipLevelCount = 1; shadowDesc.sampleCount = 1;
            m_shadowTexture = wgpuDeviceCreateTexture(m_device, &shadowDesc); m_shadowView = wgpuTextureCreateView(m_shadowTexture, nullptr);

            WGPUSamplerDescriptor sSampDesc = {}; sSampDesc.addressModeU = WGPUAddressMode_ClampToEdge; sSampDesc.addressModeV = WGPUAddressMode_ClampToEdge; sSampDesc.addressModeW = WGPUAddressMode_ClampToEdge; sSampDesc.magFilter = WGPUFilterMode_Linear; sSampDesc.minFilter = WGPUFilterMode_Linear; sSampDesc.mipmapFilter = WGPUMipmapFilterMode_Nearest; sSampDesc.maxAnisotropy = 1; sSampDesc.compare = WGPUCompareFunction_Less;
            m_shadowSampler = wgpuDeviceCreateSampler(m_device, &sSampDesc);
        }

        // --- THE FIX: Cleanly build the Post-Processing / SSAO Bind Group ---
        if (m_pipelineMgr->GetSSAOLayout() && m_resourceMgr->GetUniformBuffer()) {
            WGPUBindGroupEntry bgEntries[3] = {};
            bgEntries[0].binding = 0; bgEntries[0].buffer = m_resourceMgr->GetUniformBuffer(); bgEntries[0].size = sizeof(SceneUniforms);
    ...
    void GraphicsContext::Resize(int newWidth, int newHeight) {
        ...
        m_width = static_cast<uint32_t>(newWidth); m_height = static_cast<uint32_t>(newHeight);
        WGPUSurfaceConfiguration cfg = {}; cfg.device = m_device; cfg.format = m_surfaceFormat; cfg.usage = WGPUTextureUsage_RenderAttachment; cfg.alphaMode = WGPUCompositeAlphaMode_Opaque; cfg.width = m_width; cfg.height = m_height; cfg.presentMode = WGPUPresentMode_Fifo;
    ...
    void GraphicsContext::RenderFrame() {
        ...
        if (m_shadowTexture && vBuf && aIdxBuf && aIdxCount > 0) {
            WGPURenderPassDepthStencilAttachment shadowDepthAtt = {}; shadowDepthAtt.view = m_shadowView; shadowDepthAtt.depthClearValue = 1.0f; shadowDepthAtt.depthLoadOp = WGPULoadOp_Clear; shadowDepthAtt.depthStoreOp = WGPUStoreOp_Store;
            WGPURenderPassDescriptor shadowRpDesc = {}; shadowRpDesc.depthStencilAttachment = &shadowDepthAtt;
            ...
            for (const auto& batch : m_resourceMgr->GetSolidBatches()) { wgpuRenderPassEncoderDrawIndexed(shadowRp, batch.indexCount, 1, batch.startIndex, 0, 0); }
            wgpuRenderPassEncoderEnd(shadowRp); wgpuRenderPassEncoderRelease(shadowRp);
        }

        // 2. Main Geometry Pass (Renders to Offscreen Texture)
        WGPURenderPassColorAttachment colorAtt = {}; colorAtt.view = m_offscreenView; colorAtt.loadOp = WGPULoadOp_Clear; colorAtt.storeOp = WGPUStoreOp_Store; colorAtt.clearValue = { 0.11f, 0.11f, 0.13f, 1.0f };
        WGPURenderPassDepthStencilAttachment depthAtt = {}; depthAtt.view = m_depthView; depthAtt.depthClearValue = 1.0f; depthAtt.depthLoadOp = WGPULoadOp_Clear; depthAtt.depthStoreOp = WGPUStoreOp_Store; depthAtt.stencilClearValue = 0; depthAtt.stencilLoadOp = WGPULoadOp_Clear; depthAtt.stencilStoreOp = WGPUStoreOp_Store;
        WGPURenderPassDescriptor rpDesc = {}; rpDesc.colorAttachmentCount = 1; rpDesc.colorAttachments = &colorAtt; rpDesc.depthStencilAttachment = &depthAtt;
        ...
        if (m_resourceMgr->GetShadowBindGroup()) wgpuRenderPassEncoderSetBindGroup(rp, 1, m_resourceMgr->GetShadowBindGroup(), 0, nullptr);
        ...
        if (m_hasHighlight && !m_highlightRanges.empty() && vBuf) {
            if (m_highlightStyle == 1 && m_resourceMgr->GetLineIndexBuffer()) {
                ...
                for (const auto& range : m_highlightRanges) wgpuRenderPassEncoderDrawIndexed(rp, range.indexCount * 2, 1, range.startIndex * 2, 0, 0);
                ...
                for (const auto& range : m_highlightRanges) wgpuRenderPassEncoderDrawIndexed(rp, range.indexCount, 1, range.startIndex, 0, 0);
            }
        }
        ...
        // --- THE FIX: Pass 3 - SSAO/Blit to the Screen ---
        ...
        if (m_ssaoBindGroup && m_pipelineMgr->GetSSAOPipeline()) {
            ...
            wgpuRenderPassEncoderDraw(ssaoRp, 3, 1, 0, 0); // Fullscreen Quad!
        }
    ...
    void GraphicsContext::InitImGui(GLFWwindow* window) {
        ...
        ImFontConfig fontCfg; fontCfg.MergeMode = true; fontCfg.PixelSnapH = true; fontCfg.GlyphMinAdvanceX = 14.0f;
        static const ImWchar iconRanges[] = { 0xe000, 0xf8ff, 0 };
        io.Fonts->AddFontDefault();
        io.Fonts->AddFontFromFileTTF("fa-solid-900.ttf", 14.0f, &fontCfg, iconRanges);
        ImGui_ImplGlfw_InitForOther(window, true);
        ImGui_ImplWGPU_InitInfo wgpuInfo = {}; wgpuInfo.Device = m_device; wgpuInfo.NumFramesInFlight = 3; wgpuInfo.RenderTargetFormat = m_surfaceFormat; wgpuInfo.DepthStencilFormat = WGPUTextureFormat_Undefined;
        ImGui_ImplWGPU_Init(&wgpuInfo);
    }

    void GraphicsContext::ShutdownImGui() {
        ImGui_ImplWGPU_Shutdown(); ImGui_ImplGlfw_Shutdown(); ImGui::DestroyContext();
    }

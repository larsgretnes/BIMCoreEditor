...
namespace BimCore {
    ...
    std::shared_ptr<IfcParse::IfcFile> BimDocument::GetDatabase() { return m_database; }
    std::string BimDocument::GetFilePath() const { return m_filePath; }

    void BimDocument::LoadPropertiesFromAST(const std::string& guid) {
    ...
    std::map<std::string, PropertyInfo> BimDocument::GetElementProperties(const std::string& guid) {
        LoadPropertiesFromAST(guid);
        return m_propertyCache[guid];
    }

    std::string BimDocument::GetElementNameFast(const std::string& guid) {
    ...
    bool BimDocument::HasModifiedProperties(const std::string& guid) const {
        auto it = m_propertyCache.find(guid);
    ...
    bool BimDocument::UpdateElementProperty(const std::string& guid, const std::string& propName, const std::string& newValue) {
        LoadPropertiesFromAST(guid);
    ...
    bool BimDocument::DeleteElementProperty(const std::string& guid, const std::string& propName) {
        LoadPropertiesFromAST(guid);
        if (m_propertyCache[guid].find(propName) != m_propertyCache[guid].end()) {
    ...
    bool BimDocument::UndoElementProperty(const std::string& guid, const std::string& propName) {
        LoadPropertiesFromAST(guid);
        if (m_propertyCache[guid].find(propName) != m_propertyCache[guid].end()) {
    ...
    bool BimDocument::DeleteElement(const std::string& guid) {
    ...
    bool BimDocument::UpdateElementColor(const std::string& guid, float r, float g, float b) {
        if (!m_database) return false;
        try {
    ...
    bool BimDocument::CommitASTChanges() {
        if (!m_database) return false;
